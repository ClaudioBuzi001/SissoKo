 ...
 */
public class AnalyzerSegment implements IObserver {
	...
	public void register() {
		StorageManager.register(this, DataType.ENV_GDN.name());
	}
	...
	public void update(final IRawData inputJson) {
		...
		final IRawDataArray list = inputJson.getSafeRawDataArray(Constants.GUIDANCE_LIST);

		final DataRoot rootData = DataRoot.createMsg();
		final var serviceNode = rootData.addHeaderOfService(Constants.SEGMENT_LIST_UPDATE);
		for (int i = 0; i < list.size(); i++) {
			final IRawDataElement elem = list.get(i);
			final String guidance = elem.getSafeString(Constants.GUIDANCE).trim();
			final String status = elem.getSafeString(Constants.GUIDANCE_STATUS).trim();
