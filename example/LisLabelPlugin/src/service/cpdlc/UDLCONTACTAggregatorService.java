...
/**
 * The Class UPLINK DATALINK CONTACT AggregatorService.
 *
 */
public class UDLCONTACTAggregatorService implements IAggregatorService {
	...
	public String getServiceName() {
		/** The service name. */
		String m_serviceName = CpdlcOutputConstants.UPLINK_CPDLC_CONTACT;
	...
	public void aggregate(final IRawData inputJson, final HeaderNode dataNode) {
		String RQ_CPDLC_CONTACT_VALUE = "";
		String RQ_CPDLC_CONTACT_COLOR = "";
		String show;

		final String TCUDS = inputJson.getSafeString(CpdlcInputConstants.CPDLC_UPLINK_CONTACT_DIALOGUE_STATUS);
		...
		if (cpdlc_map_UpLink.isPresent()) {
			...
			for (int ind = 0; ind < cpdlcArray.size() && show.isEmpty(); ind++) {
				...
				if (fieldName.equals(CpdlcOutputConstants.UPLINK_CPDLC_CONTACT)) {
				...
				RQ_CPDLC_CONTACT_VALUE = valueOUT;
				RQ_CPDLC_CONTACT_COLOR = valueColor;
			}
		}

		show = (!RQ_CPDLC_CONTACT_VALUE.isEmpty() && templateStr.contains(FlightOutputConstants.TEMPLATE_CONTROLLED))
				? "forced"
				: "";

		dataNode.addLine(CpdlcOutputConstants.UPLINK_CPDLC_CONTACT, RQ_CPDLC_CONTACT_VALUE)
