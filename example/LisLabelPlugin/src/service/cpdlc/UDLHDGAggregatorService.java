...
/**
 * The Class UPLINK DATALINK HEADING AggregatorService.
 *
 */
public class UDLHDGAggregatorService implements IAggregatorService {
	...
	public String getServiceName() {
		/** The service name. */
		String m_serviceName = CpdlcOutputConstants.UPLINK_CPDLC_HDG;
	...
	public void aggregate(final IRawData inputJson, final HeaderNode dataNode) {
		String RQ_CPDLC_HDG_VALUE = "";
		String RQ_CPDLC_HDG_COLOR = "";
		String show;

		final String RMUDS = inputJson.getSafeString(CpdlcInputConstants.CPDLC_UPLINK_ROUTE_DIALOGUE_STATUS);
		...
		if (cpdlc_map_UpLink.isPresent()) {
			...
			for (int ind = 0; ind < cpdlcArray.size() && show.isEmpty(); ind++) {
				...
				if (fieldName.equals(CpdlcOutputConstants.UPLINK_CPDLC_HDG)) {
					...
					case "7":
						...
						valueOUT = ("1".equals(inputJson.getSafeString(FlightInputConstants.PWROR))) ? "?" : "#";
						valueColor = "#FFA500";
						break;
				...
				RQ_CPDLC_HDG_VALUE = valueOUT;
				RQ_CPDLC_HDG_COLOR = valueColor;
			}
		}

		show = (!RQ_CPDLC_HDG_VALUE.isEmpty() && templateStr.contains(FlightOutputConstants.TEMPLATE_CONTROLLED))
				? "forced"
				: "";

		dataNode.addLine(CpdlcOutputConstants.UPLINK_CPDLC_HDG, RQ_CPDLC_HDG_VALUE)
