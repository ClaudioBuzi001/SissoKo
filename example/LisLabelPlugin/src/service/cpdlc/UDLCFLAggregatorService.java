...
/**
 * The Class UPLINK DATALINK CFL AggregatorService.
 *
 */
public class UDLCFLAggregatorService implements IAggregatorService {
	...
	public String getServiceName() {
		/** The service name. */
		String m_serviceName = CpdlcOutputConstants.UPLINK_CPDLC_CFL;
	...
	public void aggregate(final IRawData inputJson, final HeaderNode dataNode) {
		String RQ_CPDLC_CFL_VALUE = "";
		String RQ_CPDLC_CFL_COLOR = "";
		String show;

		final String VCUDS = inputJson.getSafeString(CpdlcInputConstants.CPDLC_UPLINK_VERTICAL_DIALOGUE_STATUS);
		...
		if (cpdlc_map_UpLink.isPresent()) {
			final IRawDataArray cpdlcArray = cpdlc_map_UpLink.get().getSafeRawDataArray("CPDLC_LIST");
			String valueOUT = "";
			String valueColor = "";
			show = "";
			for (int ind = 0; ind < cpdlcArray.size() && show.isEmpty(); ind++) {
				...
				if (fieldName.equals(CpdlcOutputConstants.UPLINK_CPDLC_CFL)) {
					...
					case "7":
						...
						valueOUT = ("1".equals(inputJson.getSafeString(FlightInputConstants.PWVOR))) ? "?" : "#";
						valueColor = "#FFA500";
						break;
				...
				RQ_CPDLC_CFL_VALUE = valueOUT;
				RQ_CPDLC_CFL_COLOR = valueColor;
			}
		}

		show = (!RQ_CPDLC_CFL_VALUE.isEmpty() && templateStr.contains(FlightOutputConstants.TEMPLATE_CONTROLLED))
				? "forced"
				: "";

		dataNode.addLine(CpdlcOutputConstants.UPLINK_CPDLC_CFL, RQ_CPDLC_CFL_VALUE)
				.setAttribute(EdmModelKeys.Attributes.COLOR, RQ_CPDLC_CFL_COLOR)
