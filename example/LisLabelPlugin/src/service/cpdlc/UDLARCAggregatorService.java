...
/**
 * The Class UPLINK DATALINK ARC AggregatorService.
 *
 */
public class UDLARCAggregatorService implements IAggregatorService {
	...
	public String getServiceName() {
		/** The service name. */
		String m_serviceName = CpdlcOutputConstants.UPLINK_CPDLC_ARC;
	...
	public void aggregate(final IRawData inputJson, final HeaderNode dataNode) {
		String RQ_CPDLC_ARC_VALUE = "";
		String RQ_CPDLC_ARC_COLOR = "";
		...
		if (cpdlc_map_UpLink.isPresent()) {
			...
			for (int ind = 0; ind < cpdlcArray.size(); ind++) {
				...
				RQ_CPDLC_ARC_VALUE = valueOUT;
				RQ_CPDLC_ARC_COLOR = valueColor;
			}
		}

		String show = (!RQ_CPDLC_ARC_VALUE.isEmpty() && templateStr.contains(FlightOutputConstants.TEMPLATE_CONTROLLED))
				? "forced"
				: "";

		dataNode.addLine(CpdlcOutputConstants.UPLINK_CPDLC_ARC, RQ_CPDLC_ARC_VALUE)
