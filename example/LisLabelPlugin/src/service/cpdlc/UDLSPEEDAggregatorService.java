...
/**
 * The Class UPLINK DATALINK SPEED AggregatorService.
 *
 */
public class UDLSPEEDAggregatorService implements IAggregatorService {
	...
	public String getServiceName() {
		/** The service name. */
		String m_serviceName = CpdlcOutputConstants.UPLINK_CPDLC_SPEED;
	...
	public void aggregate(final IRawData inputJson, final HeaderNode dataNode) {
		String RQ_CPDLC_SPEED_VALUE = "";
		String RQ_CPDLC_SPEED_COLOR = "";
		String show;

		final String SCUDS = inputJson.getSafeString(CpdlcInputConstants.CPDLC_UPLINK_SPEED_DIALOGUE_STATUS);
		...
		if (cpdlc_map_UpLink.isPresent()) {
			...
			for (int ind = 0; ind < cpdlcArray.size() && show.isEmpty(); ind++) {
				...
				if (fieldName.equals(CpdlcOutputConstants.UPLINK_CPDLC_SPEED)) {
				...
				RQ_CPDLC_SPEED_VALUE = valueOUT;
				RQ_CPDLC_SPEED_COLOR = valueColor;
			}
		}

		show = (!RQ_CPDLC_SPEED_VALUE.isEmpty() && templateStr.contains(FlightOutputConstants.TEMPLATE_CONTROLLED))
				? "forced"
				: "";

		dataNode.addLine(CpdlcOutputConstants.UPLINK_CPDLC_SPEED, RQ_CPDLC_SPEED_VALUE)
